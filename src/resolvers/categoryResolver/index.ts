...
export default class CategoryResolver {
	...
	@Query(() => Category)
	@Authorized()
	async getOneCategory(@Arg('id') id: string): Promise<Category | any> {
		try {
			const category = await categoryModel.findById(id);
			if (!category) throw new ErrorHandler('Category not gound', 404);
	...
	@Authorized(['Master'])
	async createCategory(@Args() category: CategoryInput): Promise<GeneralResponse> {
		try {
	...
	@Mutation(() => GeneralResponse)
	@Authorized(['Master'])
	async updateCategory(@Args() category: CategoryInput): Promise<GeneralResponse> {
		try {
