...
export default class AuthResolver {
	...
	@Query(() => Boolean)
	async PasswordResetRequest(@Args() { email }: PasswordResetRequestArg): Promise<Boolean> {
		try {
			...
			const emailProvider = new EmailProvider({
				template: resetPasswordTemplate(`${user.firstName}`, `${DOMAIN}?code=${hash}`),
	...
	async resetPassword(@Args() reset: PasswordResetArg) {
		try {
			...
			if (password !== confirm_password) {
				throw new ErrorHandler("The password and its confirmation doen't match!", 401);
			}
			...
			// If no user end operations and return true
			if (!user) {
				throw new ErrorHandler('Error finding the user, please try later.', 404);
			}
